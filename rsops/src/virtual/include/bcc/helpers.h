...
#define BPF_F_TABLE(_table_type, _key_type, _leaf_type, _name, _max_entries, _flags)                                               \
  ...
  __attribute__((section("maps/" _table_type))) struct _name##_table_t _name = {.flags = (_flags), .max_entries = (_max_entries)}; \
...
#define BPF_QUEUE(...)                                              \
  BPF_QUEUE_STACKX(__VA_ARGS__, BPF_QUEUE_STACK4, BPF_QUEUE_STACK3) \
  ("queue", __VA_ARGS__)
...
#define BPF_QUEUESTACK_PUBLIC(_table_type, _name, _leaf_type, _max_entries, _flags) \
  ...
  __attribute__((section("maps/export"))) struct _name##_table_t __##_name

#define BPF_QUEUESTACK_SHARED(_table_type, _name, _leaf_type, _max_entries, _flags) \
  BPF_QUEUESTACK(_table_type, _name, _leaf_type, _max_entries, _flags);             \
  __attribute__((section("maps/shared"))) struct _name##_table_t __##_name
...
#define BPF_PERF_OUTPUT(_name)                        \
  ...
  __attribute__((section("maps/perf_output"))) struct _name##_table_t _name = {.max_entries = 0}
...
#define BPF_RINGBUF_OUTPUT(_name, _num_pages)        \
  ...
  __attribute__((section("maps/ringbuf"))) struct _name##_table_t _name = {.max_entries = ((_num_pages)*PAGE_SIZE)}
...
#define BPF_HISTOGRAM(...)                                \
  BPF_HISTX(__VA_ARGS__, BPF_HIST3, BPF_HIST2, BPF_HIST1) \
  (__VA_ARGS__)
...
#define BPF_LPM_TRIE(...)                                                                \
  BPF_LPM_TRIEX(__VA_ARGS__, BPF_LPM_TRIE4, BPF_LPM_TRIE3, BPF_LPM_TRIE2, BPF_LPM_TRIE1) \
  (__VA_ARGS__)
...
#define BPF_XDP_REDIRECT_MAP(_table_type, _leaf_type, _name, _max_entries) \
  ...
  __attribute__((section("maps/"_table_type))) struct _name##_table_t _name = {.max_entries = (_max_entries)}
...
#define BPF_XSKMAP(_name, _max_entries)           \
  ...
  __attribute__((section("maps/xskmap"))) struct _name##_table_t _name = {.max_entries = (_max_entries)}
...
#define BPF_SK_STORAGE(_name, _leaf_type)                                                                  \
  ...
  __attribute__((section("maps/sk_storage"))) struct _name##_table_t _name = {.flags = BPF_F_NO_PREALLOC}; \
...
#define BPF_SOCKMAP_COMMON(_name, _max_entries, _kind, _helper_name)                                      \
  ...
  __attribute__((section("maps/" _kind))) struct _name##_table_t _name = {.max_entries = (_max_entries)}; \
...
#define BPF_CGROUP_STORAGE_COMMON(_name, _leaf_type, _kind)                   \
  ...
  __attribute__((section("maps/" _kind))) struct _name##_table_t _name = {0}; \
...
static inline __attribute__((always_inline)) void bpf_tail_call_(u64 map_fd, void *ctx, int index)
{
...
static inline __attribute__((always_inline)) unsigned __int128 bpf_ntoh128(unsigned __int128 val)
{
...
static inline __attribute__((always_inline))
BCC_SEC("helpers") void bpf_dins_pkt(void *pkt, u64 off, u64 bofs, u64 bsz, u64 val)
{
...
static inline __attribute__((always_inline))
BCC_SEC("helpers") void *bpf_map_lookup_elem_(uintptr_t map, void *key)
{
  return bpf_map_lookup_elem((void *)map, key);
}

static inline __attribute__((always_inline))
BCC_SEC("helpers") int bpf_map_update_elem_(uintptr_t map, void *key, void *value, u64 flags)
{
  return bpf_map_update_elem((void *)map, key, value, flags);
}

static inline __attribute__((always_inline))
BCC_SEC("helpers") int bpf_map_delete_elem_(uintptr_t map, void *key)
{
  return bpf_map_delete_elem((void *)map, key);
}

static inline __attribute__((always_inline))
BCC_SEC("helpers") int bpf_l3_csum_replace_(void *ctx, u64 off, u64 from, u64 to, u64 flags)
{
  switch (flags & 0xf)
  {
  case 2:
...
static inline __attribute__((always_inline))
BCC_SEC("helpers") int bpf_l4_csum_replace_(void *ctx, u64 off, u64 from, u64 to, u64 flags)
{
  switch (flags & 0xf)
  {
  case 2:
...
#define BPF_PROG(name, args...)                                                          \
  ...
  __attribute__((always_inline)) static int ____##name(unsigned long long *ctx, ##args); \
  int name(unsigned long long *ctx)                                                      \
  {                                                                                      \
    int __ret;                                                                           \
                                                                                         \
    ...
    _Pragma("GCC diagnostic pop") return __ret;                                          \
  }                                                                                      \
